export class InventoryService {
    constructor(inventoryRepository, auditLogRepository, pool, AppError) {
    ...
    async getByWarehouseAndProduct(warehouseId, productId) {
        ...
        if (!entry) {
            throw new this.AppError('Inventory entry not found', 404)
        }
        return entry
    }
    ...
    async adjustStock(warehouseId, productId, quantity, type, userId, reason) {
        if (!['ADJUSTMENT_IN', 'ADJUSTMENT_OUT'].includes(type)) {
            throw new this.AppError('Invalid adjustment type', 400)
        }
