...
export class PurchaseOrdersService {
    constructor(purchaseOrdersRepository, auditLogRepository, pool) {
    ...
    async submit(id, userId) {
        ...
        if (po.status !== 'DRAFT') {
            throw new AppError(`Cannot submit PO in status ${po.status}`, 400)
        }
    ...
    async cancel(id, userId) {
        ...
        if (!['DRAFT', 'ORDERED'].includes(po.status)) {
            throw new AppError(`Cannot cancel PO in status ${po.status}`, 400)
        }
